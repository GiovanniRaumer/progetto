...
void printone(std::vector<State> const& states, int h, int q)
{
  ...
  for (auto& state : states){
    if (state.S <= state.I){
      if (state.I <= state.R) {
        std::cout << "|" << std::setw(1 + state.S/q) << "S" << std::setw((state.I - state.S)/q) << "I" << std::setw((state.R - state.I)/q) << "R\n"; 
      }
      else if (state.R <= state.S) {
        std::cout << "|" << std::setw(1 + state.R/q) << "R" << std::setw((state.S - state.R)/q) << "S" << std::setw((state.I - state.S)/q) << "I\n";
      }
      else {
        std::cout << "|" << std::setw(1 + state.S/q) << "S" << std::setw((state.R - state.S)/q) << "R" << std::setw((state.I - state.R)/q) << "I\n";
      }
    }
    else {
      if (state.S <= state.R) {
        std::cout << "|" << std::setw(1 + state.I/q) << "I" << std::setw((state.S - state.I)/q) << "S" << std::setw((state.R - state.S)/q) << "R\n"; 
      }
      else if (state.I <= state.R) {
        std::cout << "|" << std::setw(1 + state.I/q) << "I" << std::setw((state.R - state.I)/q) << "R" << std::setw((state.S - state.R)/q) << "S\n";
      }
      else {
        std::cout << "|" << std::setw(1 + state.R/q) << "R" << std::setw((state.I - state.R)/q) << "I" << std::setw((state.S - state.I)/q) << "S\n";
      }
    }
