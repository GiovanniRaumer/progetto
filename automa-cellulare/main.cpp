...
void print(pandemic::Population const &population)
{
    ...
    std::cout << "\033[2J";
    std::cout << "| S : " << N*N-population.infected()-population.recovered() << " | " << "I : " << population.infected() << " | " << "R : " << population.recovered() << " | " <<'\n';
...
int main(int argc, char *argv[])
{
    ...
    switch (argc) {
      ...
      case (argc == 6 || argc == 9):
        ...
        case(argc==9):
        if (tolower(static_cast<char>(argv[6]))!="v" || tolower(static_cast<char>(argv[6]))!="q") {
            throw std::runtime_error{"6th input must be \"v\" or \"q\""};
        }

        case (argc==9 && tolower(static_cast<char>(argv[6]))=="v"):
        if(std::atoi(argv[7])>std::atoi(argv[3])) {
            throw std::runtime_error{"the vaccination period should start before the last iteration"};
        } 
        if(std::atoi(argv[8])<0 || std::atoi(argv[8])>1) {
            throw std::runtime_error{"vaccine effectiveness must be positive and less than 1"};
        }
        int const v_begin=std::atoi(argv[7]);
        double const v_eff=std::stod(argv[8]);
        break;

        case (argc==9 && tolower(static_cast<char>(argc[6]))=="q"):
        if(std::atoi(argv[7])>std::atoi(argv[3])) {
            throw std::runtime_error{"the quarantine period should start before the last iteration"};
        }
        if(std::atoi(argv[8])<0) {
            throw std::runtime_error{"the quarantine period must be a positive number"};
        }
