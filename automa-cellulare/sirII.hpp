...
inline int alive_neighbours(Population const& population, int x, int y) {
  int counter = 0;
  for (int i : {-1, 0, 1}) {
    for (int j : {-1, 0, 1}) {
      if (i == 0 && j == 0) {
      ...
      if (x+i<=0 || (x+i>=population.side()) || y+j<=0 || y+j>=population.side()) {
        continue;
      }
      if (population.human(x + i, y + j) == Human::Alive) {
        ++counter;
      }
    }
  }
...
inline Population evolve(Population const& current) {
  ...
  for (int r = 0; r != N; ++r) {
    for (int c = 0; c != N; ++c) {
      int const m=alive_neighbours(current, r, c);
      if (current.human(r,c)==Human::Dead && m==3) {
